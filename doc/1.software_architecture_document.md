...
| CanvasStage | L1 展示 | `requestAnimationFrame` 主绘制入口；devicePixelRatio 适配；页面隐藏时暂停 | FR-004 / NFR-007 |
| PopupView | L1 展示 | Popup UI（显隐、边界约束开关）；文案走 i18n；图标走 /res | FR-005 / FR-006 / FR-009 |
| PoseStateMachine | L2 核心 | 监听 `mousemove`；`atan2` 计算并归类 8 扇区 + 中心态；输出姿态变更事件；防抖 | FR-003 |
| CanvasTransitionRenderer | L2 核心 | 帧序列播放或 crossfade；抢占/合并策略；最小过渡间隔节流 | FR-004 |
...
| 运行时框架 | **原生 JS + DOM + Canvas** | NFR-003 包体积 ≤ 2MB；NFR-008 零配置；无需重型框架 |
| 隔离方案 | **Shadow DOM**（首选）/ 独立 iframe（备选） | FR-001/NFR-009 不污染宿主页面样式与 DOM |
| 渲染技术 | **Canvas 2D + requestAnimationFrame** | FR-004 过渡动画；NFR-007 devicePixelRatio 适配 |
| 过渡策略 | **预生成帧序列**（主）/ crossfade（备） | FR-004 性能优先；复用 `gen_transition_frames.js` + `sharp` |
...
sequenceDiagram
    ...
    BA-->>I18N: zh_CN / en
    I18N-->>CS: 文案集
    CS->>RL: preload(res/move/0~8.png)
    RL->>BA: runtime.getURL('res/...')
    BA-->>RL: 绝对URL
    RL-->>CS: 帧缓存就绪
    CS->>CV: 初始绘制(默认右下角 或 记忆坐标)
    CV-->>Page: 悬浮层可见
```
...
| 静态资源包 | `res/move/**` + `res/rest/**` + `res/icons/**` | 随扩展分发，运行时无外链（NFR-004） |
| 过渡帧产物 | `res/move/transitions/**` | 构建期生成，打入扩展包 |
| 本地运行环境 | 用户浏览器 | Chrome ≥ 88 / Edge ≥ 88（NFR-006） |

### 5.3 跨平台构建与发布拓扑

1. **构建（一次性）：** `tools/gen_transition_frames.js` 离线生成过渡帧 → 写入 `res/move/transitions/`。
